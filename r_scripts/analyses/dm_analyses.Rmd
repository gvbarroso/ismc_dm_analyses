...
# for PiN/PiS, we ignore genes with ratio is above 1 
dm.tbl$PiN_PiS[dm.tbl$PiN_PiS > 1] <- NA
...
dm.lands.evolrate.bins <- ddply(dm.lands.evolrate, .variables = c("chr", "start.window"), .fun = plyr::summarize,
                               ...
                               PiN.window = sum(PiN*length.gene) / 5e+4,
                               PiS.window = sum(PiS*length.gene) / 5e+4,
                               dN.window = sum(dN*length.gene) / 5e+4,
                               dS.window = sum(dS*length.gene) / 5e+4)
...
# centering
dm.lands.evolrate.bins$thetaC <- dm.lands.evolrate.bins$theta - mean(dm.lands.evolrate.bins$theta)
dm.lands.evolrate.bins$tmrcaC <- dm.lands.evolrate.bins$tmrca - mean(dm.lands.evolrate.bins$tmrca)
dm.lands.evolrate.bins$rhoC <- dm.lands.evolrate.bins$rho - mean(dm.lands.evolrate.bins$rho)

dm.lands.evolrate.bins$bin <- 1:nrow(dm.lands.evolrate.bins)

m.dm.cds.1 <- lm(diversity ~ (thetaC + rhoC + tmrcaC + thetaC*tmrcaC), data = dm.lands.evolrate.bins)
m.dm.cds.2 <- lm(diversity ~ (thetaC + rhoC + tmrcaC + thetaC*tmrcaC + rhoC*tmrcaC), data = dm.lands.evolrate.bins)
m.dm.cds.3 <- gls(diversity ~ (thetaC + rhoC + tmrcaC + thetaC:tmrcaC),
                  data = dm.lands.evolrate.bins, cor = corAR1(0, ~bin), method = "ML")
m.dm.cds.4 <- gls(diversity ~ (thetaC + rhoC + tmrcaC + thetaC:tmrcaC + rhoC*tmrcaC),
                  data = dm.lands.evolrate.bins, cor = corAR1(0, ~bin), method = "ML")

AIC(m.dm.cds.1, m.dm.cds.2, m.dm.cds.3, m.dm.cds.4)
...
g.dm.cds <- gls(diversity ~ (thetaC + rhoC + tmrcaC + thetaC:tmrcaC),
                data = dm.lands.evolrate.bins, weights = varPower(0, ~tmrcaC|chr), cor = corAR1(0, ~bin|chr), method = "ML")
...
# rec rate results make sense
cor.test(dm.lands.evolrate.bins$PiS, dm.lands.evolrate.bins$rho, method = "spearman") 
pcor.test(dm.lands.evolrate.bins$PiS, dm.lands.evolrate.bins$rho, dm.lands.evolrate.bins$tmrca, method = "spearman") 
cor.test(dm.lands.evolrate.bins$dS, dm.lands.evolrate.bins$rho, method = "spearman") 
