...
# computes ratios
dm.tbl$PiN_PiS <- with(dm.tbl, (PiN / MeanNumberNonSynPos) / (PiS / MeanNumberSynPos))
dm.tbl$dN_dS <- with(dm.tbl, (dN / MeanNumberNonSynPosDiv) / (dS / MeanNumberSynPosDiv))
...
#hits <- findOverlaps(query = dm.lands.gr, subject = evolrate.gr, type = "any") 
...
```{r, message=FALSE, warning=FALSE}
dm.lands.evolrate <- read.table("dm_data/dm_chr_maps/dm_maps_50kb_protein_rates.tsv",
                                header = T, sep = "\t")
...
boot.tbl <- dm.lands.evolrate %>% select(PiN, theta, PiS)
boot.PiN.theta <- boot(boot.tbl, 
  statistic = function(boot.tbl, i) {
    cor.test(boot.tbl[i, "PiN"], boot.tbl[i, "theta"], method='spearman')$estimate
  },
  R = 10000
)
...
boot.PiS.theta <- boot(boot.tbl, 
  statistic = function(boot.tbl, i) {
    cor.test(boot.tbl[i, "PiS"], boot.tbl[i, "theta"], method='spearman')$estimate
  },
  R = 10000
)
...
dm.lands.evolrate$chr <- as.numeric(dm.lands.evolrate$chr)
dm.tbl.windows <- ddply(.data = dm.lands.evolrate, .variables = c("chr", "start.window"), .fun = colMeans, na.rm = T)

cor.test(dm.tbl.windows$theta, dm.tbl.windows$length.gene, method = "spearman")
